...
function Copyright() {
  ...
  class Plan extends Component{
    ...
    componentDidMount(){
      Axios.get('http://localhost:8080/examples/splans.jsp').then(response=>{
  ...
  // console.log(ev);
      ...
      Axios.get('http://localhost:8080/examples/schoolplan.jsp',{params:data}).then(response=>{
        console.log(response);
        this.setState({
...
<TextField variant="outlined" margin="normal" align="left" label="Search here" type="name"  name="t1" autoFocus 
 ...
 {/* </div> */}
  ...
  <Button onClick={this.fun.bind(this)} size="small" variant="contained" className={classes.submit} 
   color="primary" size="medium">________Search________ </Button></center>
 {/* </Grid> */}
...
<InputLabel >Vehicle ID</InputLabel>
        ...
        <FormHelperText>__All Vehicle ID's are dispalyed here__</FormHelperText>
      </FormControl>
      </div>
 <TextField variant="outlined" margin="normal" height="10" label="Enter quantity" type="name" name="t2"
 autoFocus autoComplete="Name" className={classes.d} value={this.state.t2} align="left"
 onChange={this.handleChangeFields.bind(this)}/>
   </center> 
