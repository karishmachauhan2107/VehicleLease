...
class Dallot extends Component{
    constructor(props){
        super();
        this.state={
            age:'',
    ...
    Axios.get('http://localhost:8080/examples/svallot.jsp',{params:data}).then(response=>{
...
//   const [age, setAge] = React.useState('');
  ...
  return (
    ...
    <div >
      ...
      <div>
                        <FormControl className={classes.formControl}>
        <InputLabel >School Username</InputLabel>
        <Select
        name='a'
        value={this.state.a}
          onChange={this.handleChange.bind(this)}
        >
          ...
          {    this.state.kalu.map((ag,key) => 
                    // <div key>
          <MenuItem value={ag.un}>{ag.un}</MenuItem>
          // </div>
       )}
        {/* <MenuItem value={20}>Twenty</MenuItem>
          <MenuItem value={30}>Thirty</MenuItem> */}
        </Select>
        <FormHelperText>All School usernames are dispalyed here</FormHelperText>
      </FormControl>
       </div>
       <div>
...
export default withStyles(useStyles)(Dallot);